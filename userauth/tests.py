...
class AuthTestCase(APITestCase):
    def testSingup(self):
        response = self.client.post(f"/api/auth/signup", {"email": "test@email.com", "password": "test"})
    ...
    def testNoPassword(self):
        response = self.client.post(f"/api/auth/signup", {"email": "test@email.com"})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def testLoginNoPassword(self):
        response = self.client.post(f"/api/auth/signup", {"email": "test@email.com", "password": "test"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.client.post(f"/api/auth/login", {"email": "test@email.com"})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
